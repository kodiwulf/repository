...
# -*- coding: utf-8 -*-
"""
Kodi-Wulf Dark Index Generator.
...
def root_listing_rows(repo_root: Path, items: list[dict[str, object]]) -> str:
    ...
    rows: list[str] = []
    root_zips = sorted(repo_root.glob("repository.kodi-wulf-v*.zip"))
    known_paths = [
        *root_zips,
...
def build_document(repo_root: Path, items: list[dict[str, object]]) -> str:
    ...
    root_repo_zips = sorted(repo_root.glob("repository.kodi-wulf-v*.zip"))
...
<meta name="description" content="Dunkler Kodi-Wulf Repository Index für Kodi 21 Omega">
<style>
:root {{
  --bg: #050506;
...
<main>
  ...
  <section class="panel">
    <div class="panel-head">
      <h2>Kodi URLs</h2>
      <span class="pill">for repository.kodi-wulf</span>
    </div>
...
{esc(PUBLIC_BASE_URL)}/{esc(repo_addon_link)}</div>
  ...
  <footer>Generated by Kodi-Wulf Dark Index Builder - {esc(generated_at)}</footer>
</main>
</body>
</html>
'''
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Kodi-Wulf dark index.html generator")
